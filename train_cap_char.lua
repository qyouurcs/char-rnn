...
cmd:option('-cnn_proto','cnn_models/VGG_ILSVRC_16_layers_deploy.prototxt','path to CNN prototxt file in Caffe format. Note this MUST be a VGGNet-16 right now.')
cmd:option('-cnn_model','cnn_models/VGG_ILSVRC_16_layers.caffemodel','path to CNN model file containing the weights, Caffe format. Note this MUST be a VGGNet-16 right now.')
...
-- new
cmd:option('-data_dir','data/coco','data directory. Should contain the file input.txt with input data')
...
if string.len(opt.start_from) > 0 then
  ...
  lmOpt.vocab_size = loader:getCharVocabSize()
  lmOpt.input_encoding_size = opt.input_encoding_size
  lmOpt.rnn_size = opt.rnn_size
  lmOpt.num_layers = 2
  lmOpt.dropout = opt.drop_prob_lm
  lmOpt.seq_length = loader:getSeqLengthChar()
...
local function eval_split(split, evalopt)
  ...
  local loss_evals = 0
  local predictions = {}
  local vocab = loader:getVocabChar()
  while true do
  ...
  if (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters) then
    ...
    if best_score == nil or current_score > best_score then
      ...
      if iter > 0 then -- dont save on very first iteration
        ...
        checkpoint.vocab = loader:getVocabChar()
